...
#+begin_src emacs-lisp :tangle no :noweb-ref mu4e-context
  ,(make-mu4e-context
    :name "Business"
    :enter-func (lambda () (mu4e-message "Entering Business context"))
    :leave-func (lambda () (mu4e-message "Leaving Business context"))
    ;; we match based on the maildir of the message
    :match-func (lambda (msg)
                  (when msg
                    (string-prefix-p "/business" (mu4e-message-field msg :maildir))))
    :vars
    '((user-mail-address . "john@lordpharmaconsulting.com")
      ...
      (mu4e-refile-folder . "/business/all")
      (mu4e-maildir-shortcuts
       . ((:maildir "/business/INBOX"        :key ?i)
          (:maildir "/business/sent"         :key ?s)
          (:maildir "/business/trash"        :key ?t)
          (:maildir "/business/drafts"       :key ?d)
          (:maildir "/business/all"          :key ?a)
          (:maildir "/archive"                  :key ?A)))
      (smtpmail-queue-dir . "~/mail/business/queue/cur")
